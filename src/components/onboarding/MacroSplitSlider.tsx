...
export function MacroSplitSlider({
  totalCalories,
  goals,
  onUpdate,
}: MacroSplitSliderProps) {
  // Przelicz gramy na procenty
  const proteinPercent = Math.round(
    (goals.protein * CALORIES_PER_GRAM.protein * 100) / totalCalories
  );
  const fatPercent = Math.round(
    (goals.fat * CALORIES_PER_GRAM.fat * 100) / totalCalories
  );
  const carbsPercent = Math.round(
    (goals.carbs * CALORIES_PER_GRAM.carbs * 100) / totalCalories
  );

  const totalPercent = proteinPercent + fatPercent + carbsPercent;

  const handleProteinChange = (value: number[]) => {
    const newProteinGrams = Math.round(
      (value[0] / 100) * totalCalories / CALORIES_PER_GRAM.protein
    );
    onUpdate({ protein: newProteinGrams });
  };

  const handleFatChange = (value: number[]) => {
    const newFatGrams = Math.round(
      (value[0] / 100) * totalCalories / CALORIES_PER_GRAM.fat
    );
    onUpdate({ fat: newFatGrams });
  };

  const handleCarbsChange = (value: number[]) => {
    const newCarbsGrams = Math.round(
      (value[0] / 100) * totalCalories / CALORIES_PER_GRAM.carbs
    );
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="protein-slider" className="shrink-0">Białko</Label>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="fat-slider" className="shrink-0">Tłuszcze</Label>
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Input
              id="fat-grams"
              type="number"
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor="carbs-slider" className="shrink-0">Węglowodany</Label>
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Input
              id="carbs-grams"
              type="number"
