...
export function useMealComposer(onSuccess?: () => void): UseMealComposerReturn {
  ...
  const updateCandidate = useCallback((field: keyof MealCandidateViewModel, value: any) => {
    if (!candidate) return;
  ...
  const save = useCallback(async () => {
    ...
    if (candidate.calories < 0 || candidate.protein < 0 || candidate.fat < 0 || candidate.carbs < 0 || candidate.fiber < 0) {
